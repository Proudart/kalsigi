  ...
  try {
    ...
    if (rateLimitCheck.isBlocked) {
      const headers = formatRateLimitHeaders(
        rateLimitCheck.userLimit.isLimited
          ? rateLimitCheck.userLimit
          : rateLimitCheck.ipLimit
      );

      const message = rateLimitCheck.userLimit.isLimited
        ? `Too many chapter uploads. You can upload ${
            uploadRateLimiters.chapterUpload.check(request).remaining
          } more chapters in the next hour.`
    ...
    pageFiles.forEach(([_, file]: [any, File | any], index) => {
