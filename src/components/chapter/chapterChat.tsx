...
const ChapterChat: React.FC<{ chapterId: string }> = ({ chapterId }) => {
  ...
  const fetchMessages = async () => {
    const res = await fetch(`/api/auth/chapterMessages?chapterId=${chapterId}`);
    if (res.ok) {
  ...
  const sendMessage = async (content: string, parentId: string | null = null) => {
    ...
    const res = await fetch('/api/auth/chapterMessages', {
      method: 'POST',
  ...
  const updateMessageRecursively = (messages: Message[], targetId: string, updatedData: Partial<Message>): Message[] => {
  ...
  const MessageComponent: React.FC<{ message: Message, depth: number }> = React.memo(({ message, depth }) => {
